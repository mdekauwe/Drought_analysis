#!/usr/bin/env python
"""
Plot DJFMAM for each year of the Millennium drought
"""
...
def main(fname, plot_dir):
    ...
    ofname = os.path.join(plot_dir, "DJFMAM_transpiration_anomaly.png")
...
def plot_map(ax, var, year, cmap, i, top, bottom, left, right):
    ...
    var = np.where(np.logical_and(var <= 0.1, var >= -0.1), np.nan, var)
